...
class PersonActivityMessage(db.Model):
    ...
    def send_message(activity_id, person_id_list, message):
        for person_id in person_id_list:
            person_activity_message = PersonActivityMessage.query.filter_by(activity_id=activity_id, person_id=person_id).first()
            if person_activity_message is None:
                person_activity_message = PersonActivityMessage(person_id, activity_id, message)
